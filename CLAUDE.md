...
|--------|------|-------------|----------|
| **PostgreSQL 17** | 변호사(17,326건), 법률용어(72,700건), 재판통계, 법령/판례 원본, 인제스트 21개 타입 원본, 워크스페이스(사건·대화·태그·타임라인·활동로그·구조화요약), **BM25 FTS** (pg_textsearch, 425K건 search_text + BMW 최적화) | `USE_DB_LAWYERS`, `USE_LEGAL_TERM_DICT`, `USE_BM25_SEARCH` | `backend/CLAUDE.md` |
| **LanceDB** | 법령+판례 등 21개 타입 벡터 임베딩 `legal_chunks`(656,532, `summary_type`/`article_number` 포함), 자치법규 `local_ordinance_chunks`(~240만) | `LANCEDB_MODE`, `LANCEDB_INDEX_TYPE` | `backend/CLAUDE.md` |
