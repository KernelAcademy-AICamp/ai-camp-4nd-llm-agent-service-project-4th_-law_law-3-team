...
Antigravity는 외부 AI CLI 도구(Gemini CLI, Codex CLI)를 활용할 때 이 규칙들을 **항상(ALWAYS)** 따라야 합니다.

> **관련 스킬**:
> - `.agent/skills/gemini-cli-delegation/SKILL.md` - Gemini CLI 단독 사용 패턴
> - `.agent/skills/codex-cli-delegation/SKILL.md` - Codex CLI 단독 사용 패턴
> - `.agent/skills/multi-cli-integration/SKILL.md` - CLI 조합 패턴
...
|----------|-----------|-----------------|----------|
| **대규모 코드 분석** (10+ 파일) | Gemini CLI | Antigravity (Task/Explore) | 파일 수, 컨텍스트 크기 |
| **프로젝트 아키텍처 파악** | Gemini CLI | Antigravity (Task/Explore) | 전체 구조 이해 필요 여부 |
| **보안 감사** (다수 파일) | Gemini CLI | Antigravity (순차 분석) | 분석 범위 |
| **마이그레이션 영향 분석** | Gemini CLI | Antigravity (Grep/Read) | 변경 영향 범위 |
| **멀티모달 분석** (이미지/스크린샷) | Gemini CLI | 대체 불가 | 이미지 입력 존재 여부 |
| **Deep Think 추론** (복잡 논리) | Gemini CLI | Antigravity (제한적) | GPQA 수준 추론 필요 여부 |
| **빠른 프로토타이핑** (PoC) | Gemini CLI | Antigravity (직접) | 실험적 코드 + 속도 우선 |
| **팩트 체크/최신 정보** | Gemini CLI | Codex CLI (--search) | 실시간 검증 필요 여부 |
| **코드 리뷰** (PR/커밋) | Codex CLI (/review) | Antigravity (git diff 분석) | 리뷰 대상 존재 여부 |
| **샌드박스 실행 테스트** | Codex CLI | Antigravity (코드 설명만) | 실행 필요 여부 |
| **웹 검색 + 코딩** | Codex CLI (--search) | Antigravity (WebSearch) | 외부 정보 필요 여부 |
| **수학/논리 추론** (수식 검증) | Codex CLI | Antigravity (AIME 92.8%) | 수학적 정확성 필요 여부 |
| **CI/CD 자동화** (비대화형) | Codex CLI (exec) | 대체 불가 | 자동화 파이프라인 여부 |
| **정밀 코드 작성** | Antigravity | - | 항상 Antigravity |
| **단일 파일 분석** | Antigravity | - | 항상 Antigravity |
| **5개 이하 파일 분석** | Antigravity | - | 항상 Antigravity |
...
사용자 요청 수신
    ...
    │   │         ├─ 설치됨 → Gemini CLI 실행
    │   │         └─ 미설치 → Antigravity (Task/Explore + Glob/Grep)
    │   └─ NO → 다음 판단으로
    │
    ...
    │
    ├─ 빠른 프로토타입/실험 코드?
    │   ├─ YES → Gemini CLI (제로샷) → Antigravity가 정제
    │   └─ NO → 다음 판단으로
    │
    ├─ 코드 리뷰 요청? (PR, 커밋 diff 분석)
    │   ├─ YES → Codex CLI 설치 확인
    │   │         ├─ 설치됨 → Codex CLI (/review 명령어 우선)
    │   │         └─ 미설치 → Antigravity (git diff 직접 분석)
    │   └─ NO → 다음 판단으로
    │
    ├─ 수학/논리 추론 필요? (수식 검증, 조건 완전성)
    │   ├─ YES → Codex CLI 설치 확인
    │   │         ├─ 설치됨 → Codex CLI (AIME 100%)
    │   │         └─ 미설치 → Antigravity (AIME 92.8% 수준)
    │   └─ NO → 다음 판단으로
    │
    ├─ 샌드박스 실행 필요?
    │   ├─ YES → Codex CLI 설치 확인
    │   │         ├─ 설치됨 → Codex CLI (--sandbox read-only)
    │   │         └─ 미설치 → Antigravity (실행 불가 안내 + 코드 설명)
    │   └─ NO → 다음 판단으로
    │
    ├─ 웹 검색 + 코딩 필요?
    │   ├─ YES → Codex CLI 설치 확인
    │   │         ├─ 설치됨 → Codex CLI (--search)
    │   │         └─ 미설치 → Antigravity (WebSearch 도구 사용)
    │   └─ NO → 다음 판단으로
    │
    ├─ CI/CD 자동화 필요?
    │   ├─ YES → Codex CLI (codex exec, 대체 불가)
    │   └─ NO → Antigravity 직접 수행
    │
    └─ 그 외 모든 경우 → Antigravity 직접 수행
```
...
```
[CLI명] CLI가 설치되어 있지 않아 Antigravity가 직접 분석을 수행합니다.
...
외부 CLI 실행 후 Antigravity는 **반드시** 결과를 검증합니다.
...
| 파일 수정 모드로 CLI 실행 | Antigravity만 코드 수정 담당 |
...
📊 CLI 도구 사용 보고
─────────────────────────────────────────────────
🔧 사용 도구: [Gemini CLI / Codex CLI / Fallback(Antigravity)]
...
| 도구 | 강점 | 약점 | 최적 사용 시나리오 |
|------|------|------|------------------|
| **Antigravity** | 정밀 코딩, 맥락 이해, 오케스트레이션 | 대규모 컨텍스트 제한 | 코드 작성, 계획 수립, 최종 검토 |
...
**중요**: 이 규칙들은 외부 CLI 도구 사용 시 예외 없이 적용됩니다. CLI 도구는 보조 수단이며, 최종 코드 작성과 검증은 항상 Antigravity가 담당합니다.
