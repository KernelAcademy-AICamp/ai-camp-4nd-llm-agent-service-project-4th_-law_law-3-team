...
Antigravity는 모든 코드 작성 시 이 규칙들을 **항상(ALWAYS)** 따라야 합니다.
...
관련된 스킬(`.agent/skills/`)과 에이전트(`.agent/agents/`)도 반드시 함께 업데이트해야 합니다.
...
### 확인 방법
변경된 파일 경로를 `.agent/skills/` 및 `.agent/agents/` 내에서 검색:
```bash
grep -r "변경전_경로" .agent/skills/ .agent/agents/
```
...
2. 설치되어 있지 않으면 Antigravity가 직접 수행 (Fallback)
3. 외부 CLI의 출력은 참고 자료로만 활용하며, 최종 판단과 코드 작성은 Antigravity가 수행
4. **CLI 도구 선택 기준**: `.agent/rules/cli-tool-routing.md` 참조

### 상세 가이드
- **Gemini CLI**: `.agent/skills/gemini-cli-delegation/SKILL.md`
- **Codex CLI**: `.agent/skills/codex-cli-delegation/SKILL.md`
- **CLI 조합**: `.agent/skills/multi-cli-integration/SKILL.md`
- **오케스트레이션**: `.agent/agents/ai-orchestrator.md`
