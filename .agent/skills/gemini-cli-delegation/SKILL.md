...
### 왜 위임하는가?
- Antigravity의 컨텍스트를 효율적으로 사용 (요약된 결과만 수신)
- Gemini의 **1M 토큰** 컨텍스트 윈도우를 활용한 광범위 분석 (Antigravity 200K의 5배)
- **멀티모달 처리** (이미지, 스크린샷, PDF 해석) — Antigravity Code 터미널에서 불가능한 작업
- **Deep Think 모드**로 복잡한 추론 (GPQA Diamond 93.8%)
- 분석 결과를 기반으로 Antigravity가 정밀한 코드 작성에 집중

### Antigravity 대비 Gemini 3 강점 (벤치마크 기준)

| 영역 | Gemini 3 | Antigravity | 차이 |
|------|---------|--------|------|
...
| SWE-bench Verified | 76.2-77.4% | **76.8-80.9%** | Antigravity 우위 |
| 정밀 코드 작성 | 보통 | **최적** | Antigravity 우위 |
...
### Fallback 전략
Gemini CLI 미설치 시 Antigravity가 직접 수행:
...
| 문서-코드 정합성 검증 | 문서와 실제 코드 불일치 탐지 | "Antigravity.md와 실제 코드 경로 불일치 찾기" |
...
> **주의**: 프로토타입은 Gemini가 빠르게 생성하고, Antigravity가 정제/검증하는 흐름으로 사용합니다.

### 위임하지 않는 경우
- 단일 파일 분석 또는 수정
- 5개 이하 파일에 대한 간단한 분석
- 구체적인 코드 작성 작업 (Antigravity가 더 정밀)
- 복잡한 멀티 파일 리팩토링 실행 (Antigravity가 더 정확)
- Gemini CLI 미설치 환경
...
# 6. 코드-문서 정합성 검증
gemini "Antigravity.md에 기술된 파일 경로와 클래스명이 실제 코드와 일치하는지 검증해줘. 불일치 목록을 작성해줘." -y -o text
...
2. 출력 결과를 Antigravity의 작업 컨텍스트로 사용
3. **Antigravity가 결과 검증** (파일 경로, 함수명 존재 여부 등)
4. 검증된 결과를 기반으로 구체적인 파일 읽기 및 수정 진행

### 프로토타이핑 흐름 (신규)
1. Gemini CLI로 프로토타입 코드 빠르게 생성
2. Antigravity가 코드 품질/보안/프로젝트 컨벤션 검증
3. Antigravity가 프로젝트 규칙에 맞게 정제하여 적용
...
- **검증 필수**: Gemini 출력은 반드시 Antigravity가 검증 (잘못된 경로, 존재하지 않는 함수 등 가능)
- **민감 정보 차단**: API 키, 비밀번호가 포함된 파일(`.env`, `credentials.json` 등)은 Gemini에 전달하지 않음
- **에러 시 fallback**: Gemini CLI가 에러를 반환하면 Antigravity가 직접 분석으로 전환
- **읽기 전용 분석**: `-y` (yolo) 모드는 읽기 전용 분석에만 사용, 파일 수정 작업에는 사용하지 않음
- **네트워크 의존**: Gemini CLI는 네트워크 연결 필요, 오프라인 환경에서는 Antigravity가 직접 수행
- **프로토타입 코드 직접 사용 금지**: Gemini가 생성한 코드는 반드시 Antigravity가 검증/정제 후 적용
