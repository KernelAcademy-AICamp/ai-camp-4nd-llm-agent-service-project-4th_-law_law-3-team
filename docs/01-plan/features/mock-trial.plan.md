...
- **한국 형사 공판절차**: 인정신문 → 검사 모두진술 → 피고인 의견진술 → 증거조사 → 피고인신문 → 검사 구형 → 변호인 최후변론 → 피고인 최후진술 → 판결선고 (형사소송법 제275조~제321조)
...
9. [최종변론] 검사 구형 → 변호인 최후변론 → 피고인 최후진술
...
| FR-13 | 5단계 최종변론: 검사 구형 → 변호인 최후변론 → 피고인 최후진술 | High | Pending |
| FR-13a | 6단계 판결선고: AI 판사 판결문 생성 + 피드백 | High | Pending |
...
| FR-28 | 면책 고지 표시 ("실제 법률 자문이 아닙니다") | High | Pending |
...
| LLM 모델 | Solar / OpenAI / Gemini | **Solar** (기존) + 확장 가능 | 한국어 성능 검증됨 |
...
class CourtAgent:
    ...
    profile: AgentProfile  # name, role, personality, expertise

    # Memory Module — 기억 관리
    short_term: list[str]  # 현재 단계 발언 기록
    long_term: list[str]   # 이전 단계 요약

    # Strategy Module — 전략 관리
    strategy: str          # 현재 단계 전략
    # 각 단계 종료 시 reflection → 다음 단계 전략 업데이트

    # Tools — 외부 도구
    tools: list[Tool]      # Legal Article Retriever, Case Retriever
```
...
- LangGraph: 서브그래프, interrupt
- Solar LLM: 에이전트별 LLM 호출
- LanceDB / 기존 RAG: 법률 검색 도구 (독립 인터페이스로 래핑)
...
|-------|------|------|----------|--------|
| **A. 에셋** | 1 | 픽셀아트 법정 타일맵 제작 (Tiled) | `public/assets/mock-trial/tilemap/` | - |
| | 2 | 캐릭터 스프라이트시트 제작/조달 | `public/assets/mock-trial/sprites/` | - |
| **B. Frontend** | 3 | Phaser.js 설치 + Next.js dynamic import 설정 | `package.json`, `MockTrialGame.tsx` | Step 1-2 |
| | 4 | 법정 씬 구현 (타일맵 + 캐릭터 배치) | `game/CourtScene.ts` | Step 3 |
| | 5 | 말풍선 + 캐릭터 애니메이션 | `game/ui/SpeechBubble.ts`, 스프라이트 | Step 4 |
| | 6 | EventBus (Phaser ↔ React 통신) | `game/EventBus.ts` | Step 4 |
| | 7 | React 오버레이 UI (ChatPanel, EvidencePanel) | `components/` | Step 6 |
| **C. Backend** | 8 | 스키마 정의 | `modules/mock_trial/schema/` | - |
| | 9 | CourtAgent 클래스 (Profile/Memory/Strategy) | `subgraphs/mock_trial_agents.py` | Step 8 |
| | 10 | 에이전트 프롬프트 정의 | `subgraphs/mock_trial_prompts.py` | Step 9 |
| | 11 | 서브그래프 구현 (형사 6단계 + 민사 6단계) | `subgraphs/mock_trial.py` | Step 9-10 |
| | 12 | MockTrialAgent + 라우터/노드/그래프 통합 | `agents/`, `router.py`, `nodes.py`, `graph.py` | Step 11 |
| | 13 | 모듈 라우터 + 서비스 함수 | `modules/mock_trial/router/`, `services/` | Step 11 |
| **D. RAG** | 14 | 모의재판 전용 검색 인터페이스 | `services/mock_trial_service.py` | Step 13 |
| **E. 통합** | 15 | Frontend ↔ Backend WebSocket/SSE 연동 | 전체 | Step 7, 13 |
| | 16 | 모듈 등록 + 정적 검증 | `modules.ts`, `api.ts`, 린트 | Step 15 |
...
| 단계 | 노드 | 법적 근거 | 게임 내 동작 |
|------|------|----------|-------------|
| 1. 인정신문 | `identity_node` | 형사소송법 §284 | 재판장이 피고인 인적사항 확인, 진술거부권 고지 (자동) |
| 2. 모두진술 | `opening_node` | 형사소송법 §285~§286 | 검사: 공소사실 요지 / 피고인·변호인: 의견 (사용자 입력) |
| 3. 증거조사 | `evidence_node` | 형사소송법 §290~§313 | RAG 검색 판례/법령 제시, 서증 제출, 증인신문 시뮬레이션 |
| 4. 피고인신문 | `examination_node` | 형사소송법 §296-2 | 검사/변호인이 피고인에게 질문 (사용자 참여) |
| 5. 최종변론 | `closing_node` | 형사소송법 §302~§303 | 검사 구형 → 변호인 최후변론 → 피고인 최후진술 (사용자 입력) |
| 6. 판결선고 | `verdict_node` | 형사소송법 §318-4 | AI 판사 판결문 생성 (유죄/무죄 + 양형 이유) |
...
1. [ ] Plan 리뷰 및 승인
2. [ ] 픽셀아트 에셋 확보 전략 결정 (무료 에셋 / AI 생성 / 직접 제작)
3. [ ] Phaser.js + Next.js PoC (프로토타입)
4. [ ] Design 문서 작성 (`/pdca design mock-trial`)
5. [ ] 구현 시작
