...
data/ingest_source/decisions_committee/dec_comm_국가인권위원회_v1.json 대상:
- 벡터 DB: 결정문요약 1문서=1벡터
...
from app.models.ingest.dec_human_rights_document import (  # noqa: E402
...
def _orm_factory(item: dict[str, Any]) -> DecHumanRightsDocument:
    """JSON item -> DecHumanRightsDocument 인스턴스"""
    return DecHumanRightsDocument(
        ...
        judgment_summary=item.get("판단요지"),
        classification_name=item.get("분류명"),
        ruling=item.get("주문"),
        ruling_summary=item.get("주문요지"),
        reason=item.get("이유"),
        appendix=item.get("별지"),
        full_text=item.get("결정례전문"),
...
def _orm_fulltext_fn(row: Any) -> str:
    """DecHumanRightsDocument ORM 인스턴스 -> FTS용 원문 텍스트 concat"""
    parts: list[str] = []
...
register_dec_comm(
    name="dec_human_rights",
    title_field="사건명",
    date_field="의결일자",
    committee_name=_COMMITTEE,
    source_filename="dec_comm_국가인권위원회_v1.json",
    orm_class=DecHumanRightsDocument,
