"""
개인정보보호위원회 결정례 인제스트 설정

data/ingest_source/decisions_committee/dec_comm_개인정보보호위원회_v1.json 대상:
- 벡터 DB: 결정문요약 1문서=1벡터
...
from app.models.ingest.dec_privacy_document import DecPrivacyDocument  # noqa: E402
from scripts.ingest.types._dec_comm_common import register_dec_comm  # noqa: E402

_COMMITTEE = "개인정보보호위원회"
...
def _orm_factory(item: dict[str, Any]) -> DecPrivacyDocument:
    """JSON item -> DecPrivacyDocument 인스턴스"""
    return DecPrivacyDocument(
...
def _orm_fulltext_fn(row: Any) -> str:
    """DecPrivacyDocument ORM 인스턴스 -> FTS용 원문 텍스트 concat"""
    parts: list[str] = []
...
register_dec_comm(
    ...
    source_filename="dec_comm_개인정보보호위원회_v1.json",
    orm_class=DecPrivacyDocument,
