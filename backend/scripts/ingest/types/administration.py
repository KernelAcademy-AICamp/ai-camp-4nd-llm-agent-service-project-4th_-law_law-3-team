...
data/ingest_source/administration_v1.json (34,254건)을 대상으로:
- 벡터 DB: 심판례요약 1문서=1벡터
...
from app.models.ingest.administration_document import (  # noqa: E402
...
_DEFAULT_SOURCE = DATA_DIR / "ingest_source" / "administration_v1.json"
...
def _orm_factory(item: dict[str, Any]) -> AdministrationDocument:
    """JSON item → AdministrationDocument 인스턴스"""
    return AdministrationDocument(
        serial_number=item.get("행정심판례일련번호", ""),
        case_name=item.get("사건명"),
        case_number=item.get("사건번호"),
        decision_date=_parse_date(item.get("의결일자")),
        disposition_date=_parse_date(item.get("처분일자")),
        disposition_agency=item.get("처분청"),
        adjudication_agency=item.get("재결청"),
        case_type=item.get("재결례유형명"),
        case_type_code=item.get("재결례유형코드"),
        ruling=item.get("주문"),
        claim=item.get("청구취지"),
        reason=item.get("이유"),
...
def _vector_metadata_fn(
    ...
    return create_chunk(
        data_type="행정심판례",
        source_id=str(item.get("행정심판례일련번호", "")),
...
def _orm_fulltext_fn(row: Any) -> str:
    """AdministrationDocument ORM 인스턴스 → FTS용 원문 텍스트 concat"""
    parts: list[str] = []
...
def _orm_fts_metadata_fn(row: Any) -> dict[str, Any]:
    """AdministrationDocument ORM 인스턴스 → fts_index 메타데이터 dict"""
    date_str = (
...
ADMINISTRATION_CONFIG = IngestConfig(
    name="administration",
    data_type_label="행정심판례",
    source_path=_DEFAULT_SOURCE,
    id_field="행정심판례일련번호",
    summary_field="심판례요약",
    title_field="사건명",
    orm_class=AdministrationDocument,
...
register_config(ADMINISTRATION_CONFIG)
