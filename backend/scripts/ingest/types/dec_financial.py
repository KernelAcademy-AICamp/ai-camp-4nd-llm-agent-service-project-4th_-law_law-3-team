...
data/ingest_source/decisions_committee/dec_comm_금융위원회_v1.json 대상:
- 벡터 DB: 결정문요약 1문서=1벡터
...
from app.models.ingest.dec_financial_document import DecFinancialDocument  # noqa: E402
...
def _orm_factory(item: dict[str, Any]) -> DecFinancialDocument:
    """JSON item -> DecFinancialDocument 인스턴스"""
    return DecFinancialDocument(
...
def _orm_fulltext_fn(row: Any) -> str:
    """DecFinancialDocument ORM 인스턴스 -> FTS용 원문 텍스트 concat"""
    parts: list[str] = []
...
register_dec_comm(
    ...
    source_filename="dec_comm_금융위원회_v1.json",
    orm_class=DecFinancialDocument,
