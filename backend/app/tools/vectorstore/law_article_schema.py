"""
법령 조문 LanceDB 스키마 - 별도 테이블, 12컬럼

기존 legal_chunks(10컬럼)와 별도의 law_article_chunks 테이블.
전체요약(Basic) + 조문요약(Specific)을 각각 임베딩하여
1문서 = 1(전체요약) + N(조문요약) 벡터를 저장합니다.

ID 체계:
- 전체요약: {법령ID}_overall_0
- 조문요약: {법령ID}_art_{조문번호}_0
"""
...
LAW_ARTICLE_SCHEMA = pa.schema([
    # ========== 기존 호환 필드 (10개) ==========
    pa.field("id", pa.utf8()),              # 청크 고유 ID
    pa.field("source_id", pa.utf8()),       # 법령ID
    pa.field("data_type", pa.utf8()),       # "법령" (고정)
    pa.field("title", pa.utf8()),           # 법령명_한글
    pa.field("content", pa.utf8()),         # 요약 텍스트
    pa.field("vector", pa.list_(pa.float32(), VECTOR_DIM)),
    pa.field("source_name", pa.utf8()),     # 소관부처명
    pa.field("chunk_index", pa.int32()),    # 0: 전체요약, 1~N: 조문요약
    pa.field("total_chunks", pa.int32()),   # 1 + 조문 수
    pa.field("date", pa.utf8()),            # 시행일자
...
class LawArticleChunk(BaseModel):
    """법령 조문 청크 모델"""
...
def create_overall_summary_chunk(
    ...
    """전체요약(Basic) 청크 생성"""
    return create_law_article_chunk(
        source_id=source_id,
        title=title,
        content=content,
        vector=vector,
