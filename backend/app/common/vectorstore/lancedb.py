...
class LanceDBStore(VectorStoreBase):
    ...
    def add_law_documents(
        ...
        Args:
            ...
            titles: 법령명 목록
            contents: 청크 텍스트 목록
            enforcement_dates: 시행일 목록
            departments: 소관부처 목록
            total_chunks_list: 총 청크 수 목록
            promulgation_dates: 공포일자 목록
            promulgation_nos: 공포번호 목록
            law_types: 법령 유형 목록
            article_nos: 조문 번호 목록
        """
        if not source_ids:
            return
    ...
    def add_precedent_documents(
        ...
        Args:
            ...
            titles: 사건명 목록
            contents: 청크 텍스트 목록
            decision_dates: 선고일 목록
            court_names: 법원명 목록
            total_chunks_list: 총 청크 수 목록
            case_numbers: 사건번호 목록
            case_types: 사건 유형 목록
            judgment_types: 판결 법원부 목록
    ...
    def add_documents(
        ...
        기존 ChromaDB 스타일 API 호환성을 위해 제공됩니다.
        새 코드에서는 add_law_documents() 또는 add_precedent_documents() 사용을 권장합니다.

        Args:
            ids: 문서 ID 목록 (source_id_chunkIndex 형식)
            embeddings: 임베딩 벡터 목록
            metadatas: 메타데이터 목록
            documents: 문서 텍스트 목록
        """
...
class LanceDBCollectionAdapter:
    ...
    기존 create_embeddings.py 등의 코드와 호환성 유지용
    """
