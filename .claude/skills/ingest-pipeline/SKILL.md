...
description: Config-driven 인제스트 파이프라인. 20개 데이터 타입의 DB 적재, 벡터 임베딩, FTS 빌드, ANN 인덱스 일괄 처리. 데이터 임베딩, 벡터 DB 구축, 데이터 적재 시 사용.
---

# 인제스트 파이프라인 스킬

Config-driven 인제스트 파이프라인. 20개 데이터 타입의 DB 적재, 벡터 임베딩, FTS 빌드, ANN 인덱스를 일괄 처리합니다.
...
| 데이터 타입 | 20개 (법령, 판례, 행정규칙, 헌재결정례, 조약, 위원회 결정문 등) |
| 총 건수 | ~584,600건 |
...
| 단계 | 설명 | 의존성 |
|------|------|--------|
| `db` | JSON → PostgreSQL ORM + FTS tsvector 동시 적재 (명사만: NNG+NNP, 2자 이상) | PostgreSQL, Alembic, MeCab |
| `vector` | JSON → LanceDB 벡터 임베딩 (1문서=1벡터, 요약문 기반) | 임베딩 모델, PyTorch |
| `fts` | PostgreSQL ORM에서 읽어 tsvector만 재빌드 (명사만: NNG+NNP, 2자 이상) | PostgreSQL, MeCab |
...
├── sources.yaml        # 20개 타입 소스 경로 (YAML)
...
| `interpretation_ministry` | 부처해석례 | 37,455 | 디렉토리 (33개) |
| `special_admin_appeal` | 특별행정심판례 | 149,073 | 디렉토리 (4개) |
...
7. `data/ingest_source/`에 JSON 소스 배치
